...
export class AcmeAgent extends Agent {
    ...
    const credentialDefinitionResult =
      await anonCredsApi.registerCredentialDefinition({
        credentialDefinition: {
          ...
          schemaId: schemaResult.schema.schemaState.schemaId,
        },
        options: {},
      });
  ...
  checkSchema_LiteralValue = async (anonCredsApi: any) => {
    ...
    if (checkSchemaExist) {
      ...
      return { schema: checkSchemaExist, type: 'AnonCredsSchemaRecord', flag: true };
    }
    return {schema: 'none', type: 'none', flag: false}
  }
  checkSchema_NotLiteralValue = async (anonCredsApi: any, schema: any) => {
  ...
  registerSchema = async () => {
    ...
    var checkSchema_flag = {schema: 'none', type: 'none', flag: false};
    checkSchema_flag = (await this.checkSchema_LiteralValue(anonCredsApi))
    // checkSchema_flag = (await this.checkSchema_NotLiteralValue(anonCredsApi, schema))
    if(checkSchema_flag["flag"]) return {schema: checkSchema_flag["schema"], type: checkSchema_flag["type"]};
